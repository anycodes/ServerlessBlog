...
window.tagcloud = (function (win, doc) { // ns
    ...
    return function (options) { // factory
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                if (JSON.parse(xmlhttp.responseText)["error"]) {
                    ...
                    var tagsList = JSON.parse(xmlhttp.responseText)["message"]
                    var tags = document.getElementById("tags");
                    var tempTagsinnerHTML = ''
                    for (var i = 0; i < tagsList.length; i++) {
                        if(thisTag && String(thisTag) == String(tagsList[i].id) ){
                            document.title = `标签：${tagsList[i].name} 列表- ${document.title}`
                        }
                        tempTagsinnerHTML += `<a href="list.html?tag=${tagsList[i].id}"> ${tagsList[i].name}</a>`

                    }
                    document.getElementById("tags").innerHTML = tempTagsinnerHTML
        ...
        var categoryUrl = baseUrl + tagsUri
